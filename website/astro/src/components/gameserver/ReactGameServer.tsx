...
export function ReactGameServer() {
  const { status, logs, session, connect, disconnect, send, checkStatus, clearLogs } = useGS();
  ...
  return (
    <div className="font-mono p-5 bg-[#1a1a1a] text-[#00ff00] min-h-[400px]">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className={clsx(
          "px-3 py-1 rounded",
          status.status === 'connected' && "bg-green-900 text-green-200",
          status.status === 'connecting' && "bg-yellow-900 text-yellow-200",
          status.status === 'disconnected' && "bg-gray-700 text-gray-300",
      ...
      {session && (
        <div className="mb-4 text-sm">
          <span className="text-yellow-400">User:</span> {session.user?.email || 'Unknown'}
        </div>
      )}
