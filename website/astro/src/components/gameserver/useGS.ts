...
export function useGS() {
  ...
  const addLog = useCallback((message: string, type: GameServerLog['type'] = 'info') => {
    setLogs(prev => [...prev, {
    ...
    (async () => {
      try {
        ...
        if (s?.session) {
          ...
          addLog(`Authenticated as: ${s.session.user?.email || 'Unknown'}`, 'success');
        } else {
          setSession(null);
          addLog('Not authenticated - sign in first at /auth/login', 'error');
        }
        ...
        const unsubStatus = supa.onWebSocketStatus((wsStatus: any) => {
          ...
          if (wsStatus.status === 'connected') {
            ...
            addLog(`WebSocket error: ${wsStatus.error || 'Unknown'}`, 'error');
          }
        });

        // Listen for WebSocket messages
        const unsubMessage = supa.onWebSocketMessage((message: WSMessage) => {
          if (!mounted) return;
  ...
  const send = useCallback(async (message: WSMessage) => {
    if (!supaRef.current) {
      addLog('Supabase not initialized', 'error');
      return;
    }
