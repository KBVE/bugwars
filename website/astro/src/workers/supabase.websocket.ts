  ...
  try {
    ...
    ws.onclose = (event) => {
      ...
      if (event.code !== 1000 && wsReconnectAttempts < WS_MAX_RECONNECT_ATTEMPTS) {
        wsReconnectAttempts++;
        console.log(`[WebSocket Worker] Reconnecting (${wsReconnectAttempts}/${WS_MAX_RECONNECT_ATTEMPTS})...`);

        wsReconnectTimer = setTimeout(() => {
          connectWebSocket(wsUrl).catch(err => {
            console.error('[WebSocket Worker] Reconnection failed:', err);
          });
...
function startHeartbeat() {
  ...
  wsHeartbeatTimer = setInterval(() => {
    ...
    try {
      ...
      console.error('[WebSocket Worker] Failed to send heartbeat:', error);
    }
    ...
    switch (type) {
      ...
      case 'init': {
        ...
        const storage = {
          ...
          setItem: (key: string, value: string) => comm.setState(key, value),
