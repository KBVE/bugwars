// src/lib/gateway/strategies/DirectStrategy.ts
 ...
 * - Uses IDBStorage for session persistence
...
export class DirectStrategy implements ISupabaseStrategy {
  ...
  async connectWebSocket(wsUrl?: string): Promise<WebSocketStatus> {
    if (this.ws && (this.ws.readyState === WebSocket.CONNECTING || this.ws.readyState === WebSocket.OPEN)) {
      console.log('[DirectStrategy] WebSocket already connected');
    ...
    this.ws.onopen = () => {
      console.log('[DirectStrategy] WebSocket connected');
    ...
    this.ws.onerror = (error) => {
      console.error('[DirectStrategy] WebSocket error:', error);
      this.emit('ws.status', { status: 'error', error: 'Connection error' });
    };

    this.ws.onclose = (event) => {
      console.log('[DirectStrategy] WebSocket closed:', event.code, event.reason);
      this.ws = null;
      ...
      if (event.code !== 1000 && this.wsReconnectAttempts < this.WS_MAX_RECONNECT_ATTEMPTS) {
        ...
        console.log(`[DirectStrategy] Reconnecting (${this.wsReconnectAttempts}/${this.WS_MAX_RECONNECT_ATTEMPTS})...`);

        this.wsReconnectTimer = setTimeout(() => {
          this.connectWebSocket(wsUrl).catch(err => {
            console.error('[DirectStrategy] Reconnection failed:', err);
          });
  ...
  async disconnectWebSocket(): Promise<void> {
    ...
    if (this.ws) {
      console.log('[DirectStrategy] Disconnecting WebSocket');
  ...
  private startHeartbeat() {
    ...
    this.wsHeartbeatTimer = setInterval(() => {
      ...
      const now = Date.now();
      if (this.wsLastPongTime > 0 && now - this.wsLastPongTime > this.WS_HEARTBEAT_TIMEOUT_MS) {
