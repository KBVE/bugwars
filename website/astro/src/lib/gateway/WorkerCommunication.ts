  ...
  private async initBroadcastChannel() {
    try {
      ...
      if (typeof BroadcastChannel !== 'undefined') {
        ...
        this.bc.onmessage = (e: MessageEvent) => this.handleBroadcast(e.data);
        this.usesPolyfill = false;
      } else {
        // Fallback to polyfill (will be dynamically imported if needed)
        console.warn('[WorkerCommunication] Native BroadcastChannel unavailable, will use polyfill if available');
        // For now, continue without BroadcastChannel (can add polyfill import later)
      }
    } catch (err) {
      console.error('[WorkerCommunication] Failed to initialize BroadcastChannel:', err);
    }
  }
    ...
    if (listeners) {
      listeners.forEach(cb => {
        try {
          ...
          console.error(`[WorkerCommunication] Listener error for ${event}:`, err);
        }
      });
    }
  }
...
export function getWorkerCommunication(channelName?: string): WorkerCommunication {
  if (!_instance) {
